...
% calculate first eccentricity squared (ee squared)
ES=1-(B^2)/(A^2);
% calculate second eccentricity squared (ee prime squared)
EPS= ES/(1-ES);

% check coordinate array sizes
NN=max(size(lat));
if (NN ~= max(size(long)))
...
if (geodes)
    ...
    if (Ngd1>1)
        ...
        DTANPHI=(1.0+EPS)*(1.0 - (ES)*C3.*(sin(PHI1)*wns)./DSINPSI).*tan(PSI);
    ...
    if (Ngeodes-Ngd1>1)
        ...
        DTANPHI=(1.0+EPS)*(1.0 - (ES)*C3.*(sin(PHI2)*wns)./DSINPSI).*tan(PSI);
        % compute output latitude (phi) and long (xla) in radians
        % I believe these are absolute, and don't need source coords added
